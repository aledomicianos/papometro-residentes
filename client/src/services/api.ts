...
import type { Patient, RequiredTask, Resident, Surgery } from '../types';
...
export const surgeriesApi = {
  ...
  create: (data: Omit<Surgery, 'id' | 'resident' | 'createdAt' | 'updatedAt'>) =>
    api.post<Surgery>('/surgeries', data).then((r) => r.data),
  update: (id: string, data: Partial<Omit<Surgery, 'id' | 'resident' | 'createdAt'>>) =>
    api.put<Surgery>(`/surgeries/${id}`, data).then((r) => r.data),
  remove: (id: string) => api.delete(`/surgeries/${id}`).then((r) => r.data),
};
...
// ─── Required Tasks ─────────────────────────────────────────────────────────
export const requiredTasksApi = {
  list:   () => api.get<RequiredTask[]>('/required-tasks').then((r) => r.data),
  create: (data: Omit<RequiredTask, 'id' | 'createdAt' | 'updatedAt'>) =>
    api.post<RequiredTask>('/required-tasks', data).then((r) => r.data),
  update: (id: string, data: Partial<Omit<RequiredTask, 'id'>>) =>
    api.put<RequiredTask>(`/required-tasks/${id}`, data).then((r) => r.data),
  remove: (id: string) => api.delete(`/required-tasks/${id}`).then((r) => r.data),
};
