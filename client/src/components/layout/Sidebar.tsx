...
const NAV_ITEMS: { key: NavTab; label: string; icon: keyof typeof ICONS }[] = [
  ...
  { key: 'obligations', label: 'Obrigações',   icon: 'obligations' },
];

// On mobile bottom bar: show 5 most used + collapse last 2 into "Mais"
...
function DesktopSidebar({ activeTab, onTabChange }: SidebarProps) {
  return (
    <aside style={{
      ...
      {/* Divider label */}
      <div style={{ fontSize: 10, fontWeight: 700, color: COLORS.slateLight, letterSpacing: 1.5, textTransform: 'uppercase', padding: '0 12px', marginBottom: 4 }}>
        Principal
      </div>

      {NAV_ITEMS.slice(0, 6).map((item) => <NavButton key={item.key} item={item} activeTab={activeTab} onTabChange={onTabChange} />)}

      <div style={{ borderTop: `1px solid ${COLORS.cardBorder}`, margin: '10px 0', opacity: 0.5 }} />
      <div style={{ fontSize: 10, fontWeight: 700, color: COLORS.slateLight, letterSpacing: 1.5, textTransform: 'uppercase', padding: '0 12px', marginBottom: 4 }}>
        Configurações
      </div>
      {NAV_ITEMS.slice(6).map((item) => <NavButton key={item.key} item={item} activeTab={activeTab} onTabChange={onTabChange} />)}
...
// ── Mobile bottom tab bar (5 main + "Mais" flyup) ────────────────────────────
function MobileTabBar({ activeTab, onTabChange }: SidebarProps) {
  const moreActive = MOBILE_MORE.some((i) => i.key === activeTab);
  ...
  return (
    <>
      ...
      {moreActive && (
        <div style={{
          position: 'fixed', bottom: 58, left: 0, right: 0, zIndex: 99,
          ...
          {MOBILE_MORE.map((item) => {
            ...
            return (
              <button key={item.key} onClick={() => onTabChange(item.key)} style={{
                ...
                gap: 4, padding: '10px', borderRadius: 12, border: 'none', cursor: 'pointer',
                background:   isActive ? `${COLORS.teal}22` : '#0B1F3A',
                color:        isActive ? COLORS.tealLight   : COLORS.slateLight,
      ...
      <nav style={{
        ...
        {/* "Mais" button for hidden tabs */}
        <button onClick={() => onTabChange(moreActive ? activeTab : 'reports')} style={{
          flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center',
          justifyContent: 'center', padding: '8px 2px', border: 'none',
          background: 'transparent', cursor: 'pointer', gap: 3, position: 'relative',
          color: moreActive ? COLORS.tealLight : COLORS.slateLight,
        }}>
          {moreActive && (
            <div style={{ position: 'absolute', top: 0, left: '15%', right: '15%', height: 2, background: COLORS.teal, borderRadius: '0 0 2px 2px' }} />
          )}
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={moreActive ? COLORS.tealLight : COLORS.slateLight} strokeWidth="1.8" strokeLinecap="round">
            <circle cx="5" cy="12" r="1" /><circle cx="12" cy="12" r="1" /><circle cx="19" cy="12" r="1" />
          </svg>
          <span style={{ fontSize: 9, fontWeight: moreActive ? 700 : 500, fontFamily: "'DM Sans', sans-serif" }}>
            Mais
          </span>
        </button>
      </nav>
