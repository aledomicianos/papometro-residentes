...
export function Dashboard() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 14 : 24 }}>
      ...
      {nextRes && (
        <div style={{ ...card, borderLeft: `4px solid ${COLORS.teal}`, background: `linear-gradient(120deg, ${COLORS.cardBg}, #0D3049)` }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: `${COLORS.teal}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Icon d={ICONS.rotate} size={20} color={COLORS.tealLight} />
            </div>
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: COLORS.teal, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 6 }}>Sugestão de Distribuição</div>
              <div style={{ fontSize: isMobile ? 13 : 15, color: COLORS.cream, lineHeight: 1.6 }}>
                ...
                {lastRes && <span style={{ color: COLORS.slateLight }}> — último foi {lastRes.label}</span>}
              </div>
            </div>
          </div>
        </div>
      ...
      <div>
        <div style={{ fontSize: isMobile ? 15 : 18, fontWeight: 700, color: COLORS.cream, marginBottom: isMobile ? 10 : 16 }}>Resumo por Residente</div>
              ...
              <div key={r.id} style={card}>
                ...
                <div style={{ marginBottom: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ fontSize: 11, color: COLORS.slateLight }}>Tarefas obrigatórias</span>
                    <span style={{ fontSize: 11, fontWeight: 700, color: pct === 100 ? COLORS.green : COLORS.cream }}>{done}/{tasks.length}</span>
                  </div>
